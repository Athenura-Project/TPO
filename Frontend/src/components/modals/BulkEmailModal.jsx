...
const BulkEmailModal = ({ isOpen, onClose, selectedTPOs, onComplete }) => {
    ...
    const handleSendEmails = async () => {
        ...
        setLoading(true);
        try {
            const response = await api.post('/intern/bulk-send-emails', {
                tpoIds: selectedTPOs.map(t => t._id),
                subject,
                message
            });
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-5">
                            ...
                            <div className="bg-[#B8CC34]/10 rounded-xl p-4 border border-[#B8CC34]/20">
                                <div className="flex items-center gap-2 text-sm text-[#224D59]">
                                    ...
                                    <span className="font-medium">PDF Attachments Included:</span>
                                </div>
                                <p className="text-xs text-[#384022]/70 mt-1 ml-7">
                                    Brochure.pdf and Placement-Policy.pdf will be automatically attached to all emails.
                                </p>
                            </div>
                        </div>
